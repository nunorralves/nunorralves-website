...
In this short tutorial i will walk you through how you can create an easy and quick (very simple) blog website for publishing your articles using [Next.js](https://nextjs.org/) and [Markdown](https://en.wikipedia.org/wiki/Markdown).
...
If you see previous window, the starter template page, it means everything went smoothly and we are ready to proceed!
...
Also, delete first line of `_app.js`:
...
As stated in the beggining of the article, we can have two types of pre-rendering, Static Generation or Server Side Rendering. Basically we have three methods for fetching data for pre-rendering:
...
In `getStaticPaths` we define the list of paths that will be rendered at build time. We iterate over all files on `posts` folder, parse files names to define respective slugs and return path list based on those ones. We also set fallback to `false` so 404 is returned in case some page is not under this list.

Let's finnally create a markdown blog post!  
...
If you prefer you can also check or download source code [here](https://github.com/nunorralves/blog-examples-tutorials/tree/main/nextjs-markdown-blog).  
